runllf=function(Y,indice,lag,y.test,total_pred_error_lambda, best_lambda, splits_freqs_old, vip_old, obs_nr){
  ...
  model=ll_regression_forest(X,y,enable.ll.split = T, ll.split.variables = selected, ll.split.weight.penalty = T, ll.split.lambda = 1) 
...
llf.rolling.window=function(Y,nprev,indice=1,lag=1){
  ...
  for(i in nprev:1){
    ...
    if (nprev-i>moving_window_lambda){ # start counting after 20 observations
